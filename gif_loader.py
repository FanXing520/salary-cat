...
def fit_size(
    ...
    animation_rows = max(1, terminal_rows - 1)
    margin_rows = max(0, (reserved_rows - 1) // 2)
    available_lines = max(1, animation_rows - margin_rows * 2)
    available_width = max(1, terminal_columns)
    available_pixel_height = available_lines * 2
    ...
    while width > available_width or (height + 1) // 2 > available_lines:
        scale *= 0.95
        width = max(1, int(source_width * scale))
        height = max(1, int(source_height * scale))
        if scale < 0.05:
