...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Run in a separate window (default).",
    )
    parser.add_argument(
        "--terminal",
        action="store_true",
        help="Run in terminal mode instead of window.",
    )
    parser.add_argument(
        "--gif",
        default="cat.gif",
    ...
    parser.add_argument(
        ...
        help="Animation scale from 0.1 to 2.0. Smaller values make block pixels feel finer. Default: 1.0.",
    )
    parser.add_argument(
        "--no-trim",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Transparent pixel cutoff from 0 to 255. Higher removes more edge ghosts. Default: 128.",
    )
    parser.add_argument(
        "--half-block",
        action="store_true",
    ...
    parser.add_argument(
        "--no-alt-screen",
        action="store_true",
        help="Do not switch to the alternate screen buffer. Useful in IDE run consoles.",
    )
...
def run() -> int:
    ...
    if args.window and not args.terminal:
        from window_main import run as window_run
        return window_run()
