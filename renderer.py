...
class TerminalRenderer:
    ...
    def enter(self) -> None:
        ...
        prefix = ALT_SCREEN if self.use_alt_screen else ""
        self.stream.write(prefix + HIDE_CURSOR + RESET + DEFAULT_FG + DEFAULT_BG + CLEAR_SCREEN + HOME)
        self.stream.flush()

    def exit(self) -> None:
        suffix = MAIN_SCREEN if self.use_alt_screen else ""
        self.stream.write(RESET + SHOW_CURSOR + suffix)
        self.stream.flush()
    ...
    def terminal_size(self) -> tuple[int, int]:
        columns, lines = _visible_terminal_size()
        return max(1, columns), max(2, lines)
...
def _visible_terminal_size() -> tuple[int, int]:
    """Return the visible console size, not the scrollback buffer on Windows."""
    if os.name == "nt":
        try:
            import ctypes
